...
@pytest.mark.parametrize(
    "attrib", ["latitude", "longitude", "mrr", "mtp", "stf_file", "dataset"]
)
...
def test_has_iteration(pro, it_name):
    ...
    lasif.api.set_up_iteration(
        pro.comm.lasif.lasif_root, iteration="init", remove_dirs=True
    )
...
def test_set_up_iteration(pro, events):
    ...
    num_events = len(
        lasif.api.list_events(pro.comm.lasif.lasif_root, iteration="init", output=True)
    )
    if not events:
        assert num_events == 2
    else:
...
def test_move_mesh(pro, event, capsys):
    ...
    if event == event_names[1]:
        event_mesh = os.path.join(
            pro.comm.project.lasif_root, "MODELS", "EVENT_MESHES", event, "mesh.h5"
        )
...
def test_get_master_model(pro):
    ...
    model = os.path.join(pro.comm.project.lasif_root, "MODELS", "Globe3D_csem_100.h5")
