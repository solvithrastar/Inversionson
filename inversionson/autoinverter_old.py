...
class AutoInverterOld(object):
    ...
    def prepare_iteration(self, first=False, validation=False):
        ...
        if self.comm.project.AdamOpt:
            ...
            it_name = adam_opt.iteration_name
        else:
    ...
    def time_for_validation(self) -> bool:
        ...
        if self.comm.project.AdamOpt:
            adam_opt = AdamOpt(self.comm)
            iteration_number = adam_opt.iteration_number
            if (
                iteration_number == 0
    ...
    def compute_gradient_for_adam(self, task: str, verbose: str):
        ...
        adam_opt = AdamOpt(self.comm)
        adam_grad = adam_opt.raw_gradient_path
        shutil.copy(gradient, adam_grad)
        adam_opt.set_gradient_task_to_finished()
        if not os.path.exists(adam_opt.raw_update_path):
        ...
        adam_smooth = adam_opt.smooth_update_path
    ...
    def finalize_iteration(self, task, verbose):
        ...
        if self.comm.project.AdamOpt:
            ...
            task = adam_opt.inversionson_task
