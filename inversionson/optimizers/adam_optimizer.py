...
class AdamOptimizer:
    ...
    def __init__(self, inversion_root):
        ...
        self.config_file = os.path.join(self.optimization_folder, "adam_config.toml")
        ...
        self.gradient_dir = os.path.join(self.optimization_folder, "RAW_GRADIENTS")
        self.raw_update_dir = os.path.join(self.optimization_folder, "RAW_UPDATES")
        self.smooth_update_dir = os.path.join(
            self.optimization_folder, "SMOOTHED_UPDATES"
        )
        self.first_moment_dir = os.path.join(self.optimization_folder, "FIRST_MOMENTS")
        self.second_moment_dir = os.path.join(
            self.optimization_folder, "SECOND_MOMENTS"
        )
    ...
    def get_latest_time_step(self):
        ...
        tasks.sort()
        if len(tasks) < 1:
            raise Exception("No models found," "please initialize first.")
        self.time_step = int(tasks[-1].split("/")[-1].split("_")[-1].split(".")[0])
    ...
    def get_first_moment_path(self, time_step=None):
        time_step = self.time_step if time_step is None else time_step
        return os.path.join(self.first_moment_dir, f"first_moment_{time_step:05d}.h5")

    def get_second_moment_path(self, time_step=None):
        time_step = self.time_step if time_step is None else time_step
        return os.path.join(self.second_moment_dir, f"second_moment_{time_step:05d}.h5")

    def get_raw_update_path(self, time_step=None):
        """Get path to raw update"""
        time_step = self.time_step if time_step is None else time_step
        return os.path.join(self.raw_update_dir, f"raw_update_{time_step:05d}.h5")

    def get_smooth_path(self, time_step=None):
        """Get the path of the smoothed gradient"""
        time_step = self.time_step if time_step is None else time_step
        return os.path.join(self.smooth_update_dir, f"smooth_update_{time_step:05d}.h5")
    ...
    def compute_raw_update(self):
        ...
        if not task_info["gradient_completed"]:
            raise Exception(
                "Gradient must be computed first. Compute gradient"
        ...
        m_t = (
            self.beta_1
            * self.get_h5_data(self.get_first_moment_path(time_step=time_step - 1))
            + (1 - self.beta_1) * g_t
        )
        ...
        self.set_h5_data(raw_update_path, update)
    ...
    def apply_smooth_update(self):
        ...
        if not task_info["smoothing_completed"]:
            raise Exception(
                "Smooth update first and set smoothing_completed to" "True."
            )
        ...
        # Update parameters
        theta_prev = self.get_h5_data(self.get_model_path(time_step=time_step - 1))

        # normalise theta and apply update
        theta_0 = self.get_h5_data(self.get_model_path(time_step=0))
        ...
        shutil.copy(
            self.get_model_path(time_step=time_step - 1),
            self.get_model_path(time_step=time_step),
        )
        self.set_h5_data(self.get_model_path(time_step=time_step), theta_physical)
    ...
    def get_iteration_name(self, validation=None):
        """Get iteration name"""
        task_info = toml.load(self.get_latest_task())
        iteration = task_info["model"].split("/")[-1].split(".")[0]
        if validation:
            return "validation_" + iteration
        else:
            return iteration
    ...
    def get_inversionson_task(self):
        ...
        if self.get_iteration_number() >= self.max_iterations:
            print(
                "Maximum number of iterations reached. If you want to "
        ...
        if not os.path.exists(self.get_latest_task()):
            raise Exception(
                "Please ensure that the config file is filled and"
